...
def load_trained_network(net, cuda, fpath, qremove=True):
    ...
    if qremove: #qremove=True 时，移除模型中所有与量化相关的参数。
        """
        model_dict 是从预训练模型文件中加载的 state_dict。state_dict 是一个包含模型参数的字典，通常用于保存和加载模型的权重和偏置。
        model_dict.items() 是字典的一个方法，返回一个由 (键, 值) 组成的迭代器。
        键 (lname)：对应参数的名称，例如 layer1.weight 或 layer1.bias,或layer1.activation_quantizer等。
        值 (lparams)：对应参数的值，是一个 PyTorch 的 torch.Tensor 对象，包含具体的权重、偏置等数值。
        """
        model_dict = {
