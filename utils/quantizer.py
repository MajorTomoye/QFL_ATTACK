...
class Quantizer(nn.Module):
    def __init__(self, bits_precision, range_tracker,epoch,module,fixed=False):
        super().__init__()
        ...
        if fixed and epoch != 0:
            self.scale = s[module]
            self.zero_point = z[module]
    ...
    def clamp(self, inputs):
        """
        torch.clamp 会逐元素检查 input 张量中的值：
        如果值小于 min，设置为 min。
        如果值大于 max，设置为 max。
        如果值在 [min, max] 范围内，不做修改。
        """
        outputs = torch.clamp(inputs, self.min_val.to(inputs.device), self.max_val.to(inputs.device)) #将输入值限制在 min_val 和 max_val 范围内。原代码有bug,这里加了to device
        return outputs
    ...
    def forward(self, inputs):
        if not self.fixed:
            self.range_tracker(inputs,self.epoch,self.module)
            self.update_params()
        elif self.epoch==0:
            self.range_tracker(inputs,self.epoch,self.module)
            self.update_params()
...
class SymmetricQuantizer(SignedQuantizer):
    def update_params(self):
        ...
        if self.fixed:
            s[self.module] = self.scale.detach().clone()
            z[self.module] = self.zero_point.detach().clone()
