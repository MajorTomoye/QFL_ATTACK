...
from utils.trackers import MovingAverageRangeTracker,AdvancedRangeTracker
...
class QuantizationEnabler(object):
    def __init__(self, model, wmode, amode, nbits,clip_method,epoch, silent=False,fixed=False):
        self.model = model
        self.wmode = wmode
    ...
    def __enter__(self):
        # loop over the model
        ## 遍历模型中的所有模块
        module2name = {m: n for n, m in self.model.named_modules()}
        for module in self.model.modules():
            ...
            if isinstance(module, QuantizedConv2d) \
                ...
                module_name = module2name.get(module, None)
                module.enable_quantization(self.wmode, self.amode, self.clip_method,self.epoch,self.nbits,module_name,self.fixed)
...
class QuantizedConv2d(nn.Conv2d):
    ...
    """
        To enable / disable quantization functionalities
    """
    def enable_quantization(self, wmode, amode, clip_method,epoch,nbits,module,fixed=False):
        # set the internals
        ...
        track=True：启用跟踪。
        """
        wtracker = AdvancedRangeTracker(shape = (wtrack_channel, 1, 1, 1), momentum=1, clip_method=clip_method,track=True) if epoch!=0 else MovingAverageRangeTracker(shape = (wtrack_channel, 1, 1, 1), momentum=1, clip_method=clip_method,track=True)#找出每个输出通道的最大最小值，并对每个输出通道对应的权重进行量化
        atracker = MovingAverageRangeTracker(shape = (1, atrack_channel, 1, 1), momentum=1, clip_method=clip_method,track=True) #激活量化量化的是输入的激活值，这个激活值的通道在第二维，是上一层的输出通道计算来的，在这一层输入的位置量化，因此这里追踪器的量化通道值在第二维，找出每个输入通道对应的最大最小值并进行量化。

        # set the weight quantizer #设置权重量化器
        if 'asymmetric' in self.wmode:
            self.weight_quantizer = AsymmetricQuantizer(
                bits_precision=self.nbits, range_tracker=wtracker,epoch=epoch,module=module,fixed=fixed)
        ...
        if 'asymmetric' in self.amode:
            self.activation_quantizer = AsymmetricQuantizer( \
                bits_precision=self.nbits, range_tracker=atracker,epoch=epoch,module=module,fixed=False)
    ...
    def forward(self, inputs):
        if self.quantization:
            ...
            # print(self.weight.grad is None)
...
class QuantizedLinear(nn.Linear):
    ...
    def enable_quantization(self, wmode, amode,clip_method,epoch, nbits,module,fixed=False):
        ...
        # set the trackers
        # wtracker = MovingAverageRangeTracker(shape = (wtrack_channel, 1), momentum=1, clip_method=clip_method,track=True)
        wtracker = AdvancedRangeTracker(shape = (wtrack_channel, 1), momentum=1, clip_method=clip_method,track=True) if epoch!=0 else MovingAverageRangeTracker(shape = (wtrack_channel, 1), momentum=1, clip_method=clip_method,track=True)#找出每个输出通道的最大最小值，并对每个输出通道对应的权重进行量化
        atracker = MovingAverageRangeTracker(shape = (1, atrack_channel), momentum=1, clip_method=clip_method,track=True)
